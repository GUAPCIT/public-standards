...
- __directions__ `[]` — направления подготовки
  ...
  - __disabledEdu__ `bool` — не отображать направление/специальность
  ...
  - __standards__ `[]` — стандарты
    - __title__ `text[10]` — название стандарта _(vo=ФГОС ВО;vo3pp=ФГОС ВО 3++;vpo=ФГОС ВПО...)_
    - __standard_url__ `text[250]` — ссылка на сайте министерства
...
- __years__ `[]` — года приема + форма обучения <подчиненная profiles>
  ...
  - __langs__ `text[50]` — языки преподавания (Список через запятую. Например: ru,en,ch,es.). Если поле пустое, то считается, что язык русский
